...
2. 设计输入 schema 并重构 `main.swift`:硬编码 `configs` → 从 stdin JSON 读取。保留 CLI 用法,改动前后用 `swift run -c release` 对比输出无回归。
...
6. 端到端验证:本地静态服务器跑通,确认网页输出与同参数 CLI 输出一致;推送后确认 Actions 部署成功、Pages 站点可访问。
