...
4. **← 当前**。部署:html/js 已在仓库根目录(`index.html`/`app.js`/`loader.js`/`vendor/`);写 GitHub Actions workflow(装 swiftly + wasm SDK → build → 把 `.wasm` 复制到根并随静态资源打包 → 部署 Pages);仓库设置里把 Pages source 改为 "GitHub Actions"。**实测产物 `ENDBattery.wasm` 约 60MB**(完整版 Foundation SDK,未瘦身),裸下载偏大;部署阶段需评估 `-Osize` / `wasm-opt` 瘦身与 gzip/br 压缩(Pages 默认对 `application/wasm` 会压缩传输),必要时按「需求含糊先问清」与用户确认取舍。根目录的 `ENDBattery.wasm` 已加进 `.gitignore`(CI 部署时重新生成放置)。
