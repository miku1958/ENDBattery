...
4. ⬜ **集成进页面**:`app.js` 提交计算后用 `extractStepsLine(stdout)` → `parseSteps` → `renderBlueprint(model, container)` 渲染到 [index.html](../../../web/index.html) 新容器,与结果区并列。**#3 已备好**:`renderBlueprint`/`extractStepsLine`/`parseSteps` 均已从 `blueprint.js` export,`.bp-*` CSS 已在 index.html。本步只需:① index.html 加蓝图 `<section>` + `<div id="blueprint">`(放在结果 `<pre>` 区附近);② app.js `import { extractStepsLine, parseSteps, renderBlueprint } from "./blueprint.js"`,在 submit handler 拿到 `stdout` 后 `const sl = extractStepsLine(stdout); renderBlueprint(sl ? parseSteps(sl.actions) : null, els.blueprint)`,并把 `blueprint: $("blueprint")` 加进 `els`。完成后交用户浏览器 review。
