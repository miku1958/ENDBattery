...
## 第四轮:网页「分流蓝图」可视化(2026-06-08 用户确认,已完成实现,待用户浏览器 review)
...
2. ✅ **6 个示意 SVG 图标 → `web/assets/icons/*.svg`**(2026-06-09 完成)。手绘示意矢量图(逻辑示意保真度,清晰可辨,视觉自由发挥),全部 64×64 透明 viewBox、统一视觉语言(slate 机身 + 蓝色流向箭头/chevron + 暖色热能池):`thermal-pool`(熔融池 + 升腾热浪)、`splitter`(1 进 → 3 出箭头扇出,读作三分)、`merger`(3 进 → 1 出,读作三合一)、`belt-straight`(轨道 + `>>>` 方向 chevron)、`belt-curve`(L 形转弯带 + 沿弧 chevron)、`bridge`(竖带跨越横带的立体交叉,双向 chevron)。`SPRITES` 值仍是无扩展名 base name,渲染器(#3)追加 `.svg`;已把 [blueprint.js](../../../web/blueprint.js:19) 契约注释 `.png`→`.svg`。废弃的 `.scratch/icons/` PNG 裁图已 trash(`.scratch` 整目录已空删)。**验证**:6 文件 `xmllint --noout` 全 well-formed;headless Chrome 把真实文件经 `<img src>` 渲成 montage 目视确认六图清晰可辨、风格统一,且绿/红 tint 容器上透明 SVG 正常透出(证明对渲染器 add/discard tint 友好)——证据 [logs/09-blueprint-icons-montage.png](../../../logs/09-blueprint-icons-montage.png)(gitignored)。回归门全绿:`blueprint-smoke`/`loader-smoke`/`page-config-smoke` ALL PASSED、`node --check web/blueprint.js` 过。**视觉/像素规则不约束本步**(手绘示意,不与游戏截图做像素匹配)。
3. ✅ **DOM 渲染器(2026-06-09 完成)**。[web/blueprint.js](../../../web/blueprint.js) 新增 `renderBlueprint(model, container)`(+ 私有 `spriteTile`/`beltTile`):吃 #1 的 `nodes` 排成 `.bp-chain` flex 横链,相邻 tile 间插 `belt-straight` sprite,每 tile = `.bp-icon`(`<img src="assets/icons/<sprite>.svg">` + splitter 的 2/3 `.bp-badge`)+ `.bp-label`;splitter tile 按 add/discard 加 `.bp-add`/`.bp-discard` tint class(透明 SVG 透出绿/红底)。空 model → `.bp-empty` 占位、`null` 不崩、`unparsed` → `.bp-warn`(不静默丢)。`document` 仅在函数体内引用,纯核心 import 不受影响。配套 CSS(`.bp-*`)已加进 [index.html](../../../web/index.html) `<style>`。**注**:渲染器 + CSS 已就位但**尚未接进提交流程**(留给 #4),线上暂不可见。**验证**(结构契约,agent 可做):新增 [web/test/blueprint-render-smoke.mjs](../../../web/test/blueprint-render-smoke.mjs) 用最小 `document` stub 跑两个种子,断言 tile 数==节点数、belt 数==节点数-1、tile/belt 交替、首 tile thermalPool 次 tile 入口 merger、splitter 数==步数、tint 与 add/discard 一致、badge==2/3 口数、img src 均 `assets/icons/*.svg`、belt 用 belt-straight、空/`null`/`unparsed` 三边界——`node web/test/blueprint-render-smoke.mjs` 25/25 全绿。回归门全绿:`blueprint-smoke`/`loader-smoke`/`page-config-smoke` ALL PASSED、`node --check web/blueprint.js` 过、6 SVG `xmllint` well-formed。**像素/视觉 review 仍由用户负责**(2026-06-09 用户原话「我负责 review html 并给出修改内容」);本步只锁结构契约,不驱动 `chrome-cdp`。
4. ✅ **集成进页面(2026-06-09 完成实现)**。① [index.html](../../../web/index.html) 在结果 `<section>` 后加蓝图 `<section>`(`<h2>分流蓝图</h2>` + `<div id="blueprint">`);② [app.js](../../../web/app.js) `import { extractStepsLine, parseSteps, renderBlueprint } from "./blueprint.js"`、`els` 加 `blueprint: $("blueprint")`、submit handler 拿到 `stdout` 后 `const sl = extractStepsLine(stdout); renderBlueprint(sl ? parseSteps(sl.actions) : null, els.blueprint)`(结果文本写入后、setStatus 前)。**验证**(结构/接线契约,agent 可做):`node --check web/app.js`/`web/blueprint.js` 过;四个回归门全绿(`blueprint-smoke`/`blueprint-render-smoke`/`loader-smoke`/`page-config-smoke` ALL PASSED);一次性 wire-check 脚本把两个种子经**真实 wasm** `buildStdinJson → runWasm → extractStepsLine → parseSteps → renderBlueprint`(最小 document stub)跑通——`extractStepsLine` 在真实 stdout 上命中打印行、splitter tile 数 == 打印步数(4号谷地 8 / 武陵 9)、tiles 各 14(source+入口汇流+splitter+每 🟢 一个汇流器,add 步 4/3 与解析器契约一致),exitCode 0。**像素/视觉 review 仍由用户负责**(2026-06-09 用户原话「我负责 review html 并给出修改内容」);本步只锁结构/接线契约,不驱动 `chrome-cdp`。本地预览:`cp swift/.build/wasm32-unknown-wasip1/release/ENDBattery.wasm web/`(已 cp,gitignored)后 `cd web && python3 -m http.server`。
...
本仓库无 JS 构建步、build job 在 `container: swift:6.3.2` 里靠 GitHub 注入的 node 跑 JS action,故无需加 `setup-node`,只 bump 版本。沿用既有 major-only pin 风格(`@v6` 而非 `@v6.0.3`)。**离线验证**:YAML 经 ruby psych 解析通过。**端到端验证(push 后已做)**:commit `76ed6f9` 触发的 run [27214497497](https://github.com/miku1958/ENDBattery/actions/runs/27214497497) build ✓50s + deploy ✓ 全绿;`gh api .../jobs` 与各 job check-annotations **无任何 action node-version deprecation 注解**(日志里仅一条 `DEP0040 punycode` —— node24 自身内部 warning,来自 deploy-pages 依赖,非 action runtime,无须处理)。

### 顺带修复:deploy 组装漏带第四轮新增文件(deploy-breaking,已修)

排查 deploy-pages.yml 时发现 **线上站点已被第四轮 step4 打挂**:`web/app.js` 第 13 行 `import "./blueprint.js"`,但 [deploy-pages.yml](../../workflows/deploy-pages.yml) 组装步只 `cp` 了 `index.html app.js loader.js` + `vendor` + wasm,**漏了 step4 新增的 `web/blueprint.js` 与 `web/assets/`**。实测线上 `app.js` 200 但 `blueprint.js` / `assets/icons/splitter.svg` 均 **404** → ES module import 失败 → 整页脚本崩。Node 24 这次 push 会触发重部,若不一并修则我的 push 会再发一版坏站。**修复**:组装步 cp 行加 `web/blueprint.js`、新增 `cp -R web/assets _site/assets`。**离线验证**:scratch 里 dry-run 组装 `_site`,把页面引用的 10+ 路径(`blueprint.js`、6 个 `assets/icons/*.svg`、`vendor/browser_wasi_shim/index.js`、wasm 等)逐一对 `_site` 解析全部 OK。**线上验证(重部后)**:run [27214497497](https://github.com/miku1958/ENDBattery/actions/runs/27214497497) 部署后,`blueprint.js`(200 `application/javascript`)、6 个 `assets/icons/*.svg`(200 `image/svg+xml`)由原 404 全部恢复;`index.html`/`app.js`/`loader.js`/`vendor/.../index.js`/`ENDBattery.wasm`(200 `application/wasm`)整张 module 图逐条 200。**根因归属**:第四轮 step4「集成进页面」自审漏查 CI 组装波及面(新增的 import 目标 + 静态资源未进 `_site`),本轮补上。
