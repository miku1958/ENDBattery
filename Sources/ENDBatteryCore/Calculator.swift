...
// Tunable parameters are module-level state, set per run from CalculatorInput
// by applyOptions(_:) (see Input.swift). The "4号谷地" / "武陵" scenarios that
// used to be hardcoded here now live as test cases in ENDBatteryCoreTests.

/* ————————————————————————————— 选填的数据 ————————————————————————————— */

/// 最少分流电池数, 调高会过滤掉一些方案, 一般不需要改
var minAnalyzedBatteryCount: Int = 1

/// 递归时额外的传送带/分流器数量（格）, 一般不用改
/// 用于限制递归上限
/// 最小值为 1
var extraBeltInSteps: Int = 1

/// 最大递归深度, 会影响结果的长度和运行的速度, 这个值越大越慢, 但可能找到更优解
var maxDepthLimit: Int = 9
...
public func runCalculation(input: CalculatorInput) {
applyOptions(input)
